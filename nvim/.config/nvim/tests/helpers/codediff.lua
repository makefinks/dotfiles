		...
		for _, tp in ipairs(vim.api.nvim_list_tabpages()) do
			...
			if session and explorer and session.mode == "explorer" then
				...
				if
					original_bufnr
					and modified_bufnr
					and vim.api.nvim_buf_is_valid(original_bufnr)
...
function M.focus_modified_window(tabpage)
	...
	assert(
		session and session.modified_win and vim.api.nvim_win_is_valid(session.modified_win),
		"Modified CodeDiff window was not ready"
	)
...
function M.buffer_has_keymap(bufnr, lhs)
	...
	local keymap = vim.api.nvim_buf_call(bufnr, function()
		return vim.fn.maparg(lhs, "n", false, true)
	end)

	return type(keymap) == "table" and keymap.lhs ~= nil and keymap.lhs ~= ""
end

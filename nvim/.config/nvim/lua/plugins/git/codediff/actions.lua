...
local function find_current_selection(explorer, file_path, group)
	if
		explorer.current_selection
		and explorer.current_selection.path == file_path
		and explorer.current_selection.group == group
	then
...
local function set_current_selection(explorer, selection)
	if not explorer then
		return
	end

	explorer.current_file_path = selection and selection.path or nil
	explorer.current_file_group = selection and selection.group or nil
	explorer.current_selection = selection and vim.deepcopy(selection) or nil
end

local function clear_current_selection(explorer)
	if not explorer then
		return
	end

	set_current_selection(explorer, nil)
...
local function reconcile_selection(explorer, selection)
	if selection then
		set_current_selection(explorer, selection)
		return true
	end

	clear_current_selection(explorer)
...
local function rebuild_hidden_tree(explorer, status_result)
	...
	local root_nodes = tree_module.create_tree_data(
		status_result,
		explorer.git_root,
		explorer.base_revision,
		not explorer.git_root,
		explorer.visible_groups
	)
...
local function refresh_hidden_explorer(explorer, next_file)
	...
	git.get_status(explorer.git_root, function(err, status_result)
		...
		vim.schedule(function()
			...
			local updated_next_file = next_file
					and find_status_entry(filtered_status_result, next_file.path, next_file.group)
				or nil
			...
			reconcile_selection(explorer, updated_next_file)
		end)
	end)
end
...
local function refresh_after_group_action(explorer, next_file)
	if explorer and explorer.is_hidden then
		refresh_hidden_explorer(explorer, next_file)
		return
	end
...
function M.stage_entry(get_codediff_lifecycle, tabpage)
	...
	context.git.stage_file(context.explorer.git_root, context.file_path, function(err)
		...
		vim.schedule(function()
			refresh_after_group_action(context.explorer, next_file)
		end)
	end)
end
...
function M.unstage_entry(get_codediff_lifecycle, tabpage)
	...
	local next_file = get_next_file_in_group(context.explorer, context.file_path, "staged")
...
function M.open_file_picker(get_codediff_lifecycle, tabpage)
	...
	local fzf = helpers.require_module("fzf-lua", "fzf-lua is required for the codediff picker", {
		functions = "fzf_exec",
	})
	if not fzf then
		return
	...
	fzf.fzf_exec(entries, {
		...
		actions = {
			["enter"] = function(selected)
				...
				vim.schedule(function()
					view.select_explorer_file(explorer, file)
				end)
			end,
		},
